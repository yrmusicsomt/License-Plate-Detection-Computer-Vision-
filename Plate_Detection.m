...
% Plot Vertical Edge Processing Histogram
figure(6);
subplot(3,1,1);
plot(vert1);
...
for i = 2:cols-2
    if (horz(i) ~= 0 && horz(i-1) == 0 && horz(i+1) == 0)
        column(j:j+1) = [i, i];
        j = j + 2;
    elseif (horz(i) ~= 0 && (horz(i-1) == 0 || horz(i+1) == 0))
        column(j) = i;
        j = j+1;
    end
end
...
for i = 2:rows-2
    if (vert(i) ~= 0 && vert(i-1) == 0 && vert(i+1) == 0)
        row(j:j+1) = [i, i];
        j = j + 2;
    elseif (vert(i) ~= 0 && (vert(i-1) == 0 || vert(i+1) == 0))
        row(j) = i;
        j = j+1;
    end
end
...
for i = 1:2:row_size
    for j = 1:2:column_size
        if ~((max_horz >= column(j) && max_horz <= column(j+1)) && ...
             (max_vert >= row(i) && max_vert <= row(i+1)))
            for m = row(i):row(i+1)
